 			...
 			if (event.object1.myName=="chestClosed" and event.object2.myName=="rollybear") then
  				...
  				local function trophyEffect (event)
  					local function showOverlayNext()
  						...
  						storyboard.showOverlay( "gamecompleteoverlay" ,{effect = "fade"  ,  params ={curLevel = currentLevel}, isModal = true} )	
  					end
  					local function enlargeTrophy()
  						transition.to( trophy, {time=500, xScale = 2.0, yScale = 2.0, onComplete=showOverlayNext} )
  					end
  					timer.performWithDelay ( 100, enlargeTrophy )
  				end
  				transition.to( trophy, {time=500, x = centerX, y = centerY, onComplete=trophyEffect} )
  				rollybear.alpha = 0 
  			end 
  			--- events which happen when Rollbear collides to become GameOver
